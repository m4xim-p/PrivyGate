...
`/process` должен вернуть контролируемую ошибку (5xx), а не вернуть необработанный
текст как «маску». Это известное ограничение текущей реализации: явная обработка
недоступности детектора для `/process` ещё не реализована.
