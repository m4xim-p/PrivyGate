...
| Нагрузка | разгон до 1000 RPS с удержанием; в уточнении также указана средняя ~330 RPS | После ADR-0006: полный 5-мин прогон, hold-фаза ~999.5 HTTP RPS, mask p95 9ms, dropped_iterations ~0 на hold. Tombstone лимит 100k, при переполнении новые ID → 429. Цель 1000 RPS близка к достижению |
...
| Дата рождения | `DATE_OF_BIRTH` | present | Числовые (DD.MM.YYYY, YYYY-MM-DD), текстовые (пятнадцатого марта), цифровой день + месяц прописью (1 января 1980 г.); recall 0.992 на api |
